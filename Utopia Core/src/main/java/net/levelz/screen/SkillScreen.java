...
public class SkillScreen extends Screen implements Tab {
    ...
    /** Höhe, die im Panel für eine Spalte Fähigkeiten frei ist. */
    private static final int COLUMN_HEIGHT = 120;

    private final WidgetButtonPage[] skillButtons = new WidgetButtonPage[Skill.count()];
    private final WidgetButtonPage[] levelButtons = new WidgetButtonPage[Skill.count()];

    /**
     * Zwei Spalten, Zeilenabstand ergibt sich aus der Anzahl der Fähigkeiten.
     * Bei den ursprünglichen zwölf kommt exakt das alte Raster heraus (6 pro
     * Spalte, 20 px), bei mehr rücken die Zeilen zusammen, statt aus dem Panel
     * zu laufen.
     */
    private static int perColumn() {
        return (Skill.count() + 1) / 2;
    }

    private static int rowSpacing() {
        return Math.min(20, COLUMN_HEIGHT / Math.max(1, perColumn()));
    }

    private static int columnOf(int index) {
        return index < perColumn() ? 0 : 1;
    }

    private static int rowOf(int index) {
        return index - columnOf(index) * perColumn();
    }
    ...
    protected void init() {
        ...
        for (int i = 0; i < this.skillButtons.length; i++) {
            ...
            this.skillButtons[i] = this.addDrawableChild(new WidgetButtonPage(this.x + 15 + columnOf(i) * 90, this.y + 90 + rowOf(i) * rowSpacing(), 16, 16,
                    Skill.values()[i].iconIndex() * 16, 16, false, true,
            ...
            this.levelButtons[i] = this.addDrawableChild(new WidgetButtonPage(this.x + 83 + columnOf(i) * 90, this.y + 92 + rowOf(i) * rowSpacing(), 13, 13, 33, 42, true, true, null, button -> {
                int level = 1;
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (this.client.player != null) {
            ...
            for (int o = 0; o < Skill.count(); o++) {
                ...
                context.drawText(this.textRenderer, currentLevelText, this.x - this.textRenderer.getWidth(currentLevelText) / 2 + 57 + columnOf(o) * 90,
                        this.y + 95 + rowOf(o) * rowSpacing(), 0x3F3F3F, false);
            }
        }
        // Small icons
