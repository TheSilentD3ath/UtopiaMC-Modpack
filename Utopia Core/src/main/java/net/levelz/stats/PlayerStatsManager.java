...
public class PlayerStatsManager {
    ...
    // Skill - Index aus der Registry statt Map: getSkillLevel liegt in Kampf-,
    // Abbau- und Craft-Pfaden, da soll kein Hashing und kein Boxing passieren.
    private int[] skillLevel = new int[Skill.count()];
    ...
    public void readNbt(NbtCompound tag) {
        if (tag.contains("SkillPoints", 99)) {
            ...
            for (Skill stats : Skill.values()) {
                skillLevel[stats.index()] = tag.getInt(stats.getNbt());
            }
        }
    }

    /** Die Registry kann zwischen zwei Spielständen gewachsen sein. */
    private void grow() {
        if (this.skillLevel.length < Skill.count()) {
            this.skillLevel = java.util.Arrays.copyOf(this.skillLevel, Skill.count());
        }
    }
    ...
    public void writeNbt(NbtCompound tag) {
        ...
        // Skill - NBT bleibt namensbasiert ("MiningLevel"), damit alte
        // Spielstände lesbar bleiben und eine neue Fähigkeit die Reihenfolge
        // der bestehenden nicht durcheinanderbringt.
        grow();
        for (Skill stats : Skill.values()) {
            tag.putInt(stats.getNbt(), this.skillLevel[stats.index()]);
        }
    }
    ...
    public void setSkillLevel(Skill skill, int level) {
        grow();
        skillLevel[skill.index()] = level;
    }

    public int getSkillLevel(Skill skill) {
        int index = skill.index();
        return index < skillLevel.length ? skillLevel[index] : 0;
    }
    ...
    public static boolean playerLevelisHighEnough(PlayerEntity playerEntity, List<Object> list, String string, boolean creativeRequired) {
        if (!playerEntity.isCreative() || !creativeRequired) {
            // Utopia: Freischalt-Baeume. Steht die Id in einem Knoten, entscheidet
            // der Knoten - unabhaengig von Fertigkeitsstufen.
            if (string != null && string.indexOf(':') > 0) {
                net.minecraft.util.Identifier id = net.minecraft.util.Identifier.tryParse(string);
                if (id != null && dev.utopia.core.unlock.UnlockService.isLocked(playerEntity, id)) {
                    return false;
                }
            }
    ...
    public static boolean listContainsItemOrBlock(PlayerEntity playerEntity, int id, int reference) {
        ...
        // Utopia: gilt fuer jede Art von Sperrliste - ein nicht freigeschalteter
        // Knoten haelt den Gegenstand zurueck.
        if (dev.utopia.core.unlock.UnlockService.isLockedItem(playerEntity, id)) {
            return true;
        }
        return false;
    }
    ...
    public static void onLevelUp(PlayerEntity playerEntity, int playerLevel) {
        // Utopia: jeder Aufstieg gibt Freischaltpunkte fuer die Baeume - eigener
        // Topf, damit er nicht mit den Fertigkeitspunkten konkurriert.
        dev.utopia.core.unlock.UnlockService.grantForLevel(playerEntity);
    }
