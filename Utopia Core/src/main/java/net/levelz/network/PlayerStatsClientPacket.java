...
public class PlayerStatsClientPacket {
    ...
    public static void init() {
        ClientPlayNetworking.registerGlobalReceiver(PlayerStatsServerPacket.STATS_SYNC_PACKET, (client, handler, buf, sender) -> {
            ...
            client.execute(() -> {
                ...
                if (skill == Skill.STRENGTH) {
                    skill.applyAttribute(playerStatsManager.getPlayerEntity(), playerStatsManager.getSkillLevel(skill));
                }
                PlayerStatsServerPacket.syncLockedCraftingItemList(playerStatsManager);
                skill.onLevelChanged(playerStatsManager);
            });
        });
        ...
        ClientPlayNetworking.registerGlobalReceiver(PlayerStatsServerPacket.LEVEL_PACKET, (client, handler, buf, sender) -> {
            // Der Puffer muss aus dem Netzwerk-Thread heraus kopiert werden, weil
            // er nach dem Handler freigegeben wird. Vorher standen hier 16 fest
            // verdrahtete readInt() - also genau die zwölf Ursprungs-Fähigkeiten
            // plus drei Kopfwerte. Mit einer dreizehnten Fähigkeit fehlte ein
            // Wert und executeLevelPacket lief in eine IndexOutOfBoundsException.
            // Jetzt wird gelesen, was da ist.
            PacketByteBuf newBuffer = new PacketByteBuf(Unpooled.buffer());
            newBuffer.writeBytes(buf.copy());
            client.execute(() -> {
        ...
        ClientPlayNetworking.registerGlobalReceiver(PlayerStatsServerPacket.RESET_PACKET, (client, handler, buf, sender) -> {
            if (client.player != null) {
                ...
                client.execute(() -> {
                    ...
                    // Zurückgesetzt heißt Stufe 0 - Grundwert ohne Bonus.
                    skill.applyAttribute(client.player, 0);
